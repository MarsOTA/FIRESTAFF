...
import { SEQ, getMainDayCode, getMainNightCode, selectableForVigilanza } from '../utils/turnarioLogic';

export const OlympicGenerator: React.FC = () => {
  const [seedDate, setSeedDate] = useState('2026-02-14');
  const [seedCode, setSeedCode] = useState('B1');
  const [startDate, setStartDate] = useState('2026-02-14');
  const [days, setDays] = useState(14);

  // Self-test richiesto (console log)
  useEffect(() => {
    const sDate = new Date('2026-02-14T00:00:00');
    const day14 = getMainDayCode(sDate, sDate, 'B1');
    const night14 = getMainNightCode(sDate, sDate, 'B1');
    
    const d15 = new Date('2026-02-15T00:00:00');
    const day15 = getMainDayCode(d15, sDate, 'B1');
    const night15 = getMainNightCode(d15, sDate, 'B1');

    console.log("[DEBUG] SELF-CHECK TURNARIO");
    console.log("14/02: Diurno =", day14, "(Exp: B1), Notturno =", night14, "(Exp: A1)");
    console.log("15/02: Diurno =", day15, "(Exp: C1), Notturno =", night15, "(Exp: B1)");
    
    const selB1 = selectableForVigilanza('B1');
    console.log("Extra Vig. B1:", selB1.extra, "expected ['B1', 'D8', 'A1']");
  }, []);
  ...
  return (
    <div className="p-8 max-w-[1700px] mx-auto space-y-6 animate-in fade-in duration-700">
      ...
      <div className="flex justify-center pt-4">
         <div className="bg-slate-800 px-6 py-3 rounded-2xl flex gap-8 items-center border border-white/5 shadow-2xl">
            <div className="flex items-center gap-2">
               <div className="w-2 h-2 rounded-full bg-amber-400"></div>
               <span className="text-[9px] font-black text-white/60 uppercase tracking-widest">Algoritmo VVF Milano</span>
            </div>
            <div className="flex items-center gap-2">
               <div className="w-2 h-2 rounded-full bg-emerald-400"></div>
               <span className="text-[9px] font-black text-white/60 uppercase tracking-widest">Vigilanza deterministica 8+3</span>
            </div>
            <div className="flex items-center gap-2">
               <div className="w-2 h-2 rounded-full bg-red-400"></div>
               <span className="text-[9px] font-black text-white/60 uppercase tracking-widest">Notturno D-1 (Giorno Precedente)</span>
            </div>
         </div>
      </div>
    </div>
